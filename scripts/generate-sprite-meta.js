...
function parseMpf(buf) {
  ...
  const headerType = buf.readInt32LE(pos)
  pos += 4
  if (headerType === -1) {
    const extra = buf.readInt32LE(pos)
    pos += 4
    if (extra === 4) pos += 8
  } else {
    pos = 0
  ...
  /* frameCount  */ pos += 1
  const pixelWidth = buf.readInt16LE(pos)
  pos += 2
  /* pixelHeight */ pos += 2
  /* dataLength  */ pos += 4
  ...
  const formatType = buf.readInt16LE(pos)
  pos += 2
...
for (const { mpf, pngName } of pairs) {
  ...
  if (!dims) {
    errors.push(`No dimensions: ${pngName}`)
    continue
  }
