  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (trimmed === '[MoonSharpUserData]') {
      ...
      for (let j = i + 1; j < lines.length; j++) {
        ...
        const classMatch = next.match(
          /public\s+(?:sealed\s+|abstract\s+|static\s+)*(?:class|record)\s+(\w+)(?:\s*:\s*(.+))?/
        )
        if (classMatch) {
          currentClass = {
            name: classMatch[1],
    ...
    const propMatchMultiLine =
      !propMatch &&
      propLine.match(/^public\s+(?:new\s+|override\s+|virtual\s+|static\s+)*(\S+)\s+(\w+)\s*$/)
    const resolvedPropMatch =
      propMatch ||
      (propMatchMultiLine && i + 1 < lines.length && lines[i + 1].trim().startsWith('{')
        ? propMatchMultiLine
        : null)
...
function generateTodo(allClasses) {
  ...
  for (const cls of allClasses) {
    ...
    for (const method of cls.methods) {
      if (!method.doc.summary) {
        gaps.push({
          member: `${method.name}(${method.params.length} params)`,
          reason: 'Missing method summary'
        })
      }
      for (const param of method.params) {
        if (!method.doc.params[param.name]) {
          gaps.push({
            member: `${method.name} → ${param.name}`,
            reason: 'Missing param description'
          })
        }
      }
  ...
  lines.push(
    `**${totalGaps}** documentation gaps across **${allClasses.length}** Lua-exposed types.`
  )
  lines.push('')
