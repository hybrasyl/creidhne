...
describe('assembleFormula — basic substitution', () => {
  ...
  it('substitutes setting values', () => {
    const result = assembleFormula('SOURCELEVEL / {LevelDiv}', { LevelDiv: 10 }, [
      settDef('LevelDiv')
    ])
  ...
  it('substitutes coefficient values', () => {
    const result = assembleFormula('damage * {Coefficient}', { Coefficient: 1.03 }, [
      coefDef('Coefficient')
    ])
...
describe('assembleFormula — rand', () => {
  ...
  it('substitutes rand with multiplier of 1 as just the variable', () => {
    const result = assembleFormula(
      'base + {Rand}',
      { Rand: { variable: 'RAND_10', multiplier: 1 } },
      [randDef('Rand')]
    )
  ...
  it('strips empty rand term from formula', () => {
    const result = assembleFormula('base + {Rand}', { Rand: { variable: '', multiplier: 1 } }, [
      randDef('Rand')
    ])
...
describe('assembleFormula — stat_block', () => {
  it('substitutes stat block rows', () => {
    ...
    const result = assembleFormula('({StatBlock}) / 1500', { StatBlock: rows }, [
      statDef('StatBlock')
    ])
  ...
  it('substitutes empty stat block as 0', () => {
    const result = assembleFormula('({StatBlock}) / 1500', { StatBlock: [] }, [
      statDef('StatBlock')
    ])
...
describe('assembleFormula — optional rand stripping', () => {
  it('strips + {Rand} when rand has no variable', () => {
    ...
    const result = assembleFormula(template, { Base: 1, Rand: { variable: '', multiplier: 1 } }, [
      numDef('Base'),
      randDef('Rand')
    ])
  ...
  it('keeps rand when variable is set', () => {
    ...
    const result = assembleFormula(
      template,
      { Base: 1, Rand: { variable: 'RAND_10', multiplier: 4 } },
...
describe('assembleFormula — optional weapon stripping', () => {
  ...
  it('strips weapon term when _weaponEnabled is false', () => {
    ...
    const result = assembleFormula(template, { Base: 1, _weaponEnabled: false }, [
      numDef('Base'),
      weaponDef
    ])
  ...
  it('keeps weapon term when _weaponEnabled is true', () => {
    const template = '({Base} * SOURCELEVEL + SOURCEWEAPONSMALLDAMAGE * {WeaponCoeff})'
    const result = assembleFormula(template, { Base: 1, _weaponEnabled: true, WeaponCoeff: 2 }, [
      numDef('Base'),
      weaponDef
    ])
...
describe('assembleFormula — full patterns', () => {
  ...
  it('assembles New Hybrasyl with no rand and no weapon', () => {
    ...
    const result = assembleFormula(template, params, defs)
    expect(result).toBe('(1 * MAX(SOURCELEVEL - 1, 1)) * (1 + SOURCESTR * 3 / 1500) * 1')
  })
