...
export const CASTABLE_DESCRIPTION_CLASSES = [
  'Peasant',
  'Warrior',
  'Rogue',
  'Wizard',
...
export const DEFAULT_CASTABLE = {
  name: '',
  comment: '',
  meta: { isTest: false, isGM: false, givenViaScript: false, deprecated: false, specialty: '' },
  lines: '',
  cooldown: '',
  icon: '',
  book: 'PrimarySkill',
...
export function derivePrefix(classValue, book) {
  ...
  const classPart =
    classes.length === 1
      ...
      : classes.length === ALL_CASTABLE_CLASSES.length
        ? 'all'
        : ''
