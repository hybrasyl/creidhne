  ...
  return (
    <Box>
      ...
      {rows.map((row, index) => {
        ...
        const options = allForPrefix.filter((s) => s.key === row.stat || !usedStats.includes(s.key))
        ...
        return (
          <Box key={index} sx={{ display: 'flex', gap: 1, alignItems: 'center', mb: 0.75 }}>
            ...
            <Autocomplete
              ...
              onChange={(_, val) => {
                if (val) updateStat(index, val.key)
              }}
            ...
            <TextField
              ...
              value={row.weight}
              onChange={(e) =>
                updateWeight(index, e.target.value === '' ? '' : Number(e.target.value))
              }
            />
