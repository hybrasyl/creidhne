    ...
    <Paper variant="outlined" sx={{ px: 2, py: 1 }}>
      ...
      {total === 0 ? (
        ...
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
          {castables.map((n) => (
            <Chip key={`c-${n}`} size="small" label={n} variant="outlined" />
          ))}
          {statuses.map((n) => (
            <Chip key={`s-${n}`} size="small" label={n} variant="outlined" color="secondary" />
          ))}
        </Box>
      )}
    </Paper>
...
function FormulaEditor({
  ...
  const castableAcquiredLevel = useMemo(
    () =>
      (refType === 'castable' && refName
        ? libraryIndex?.castableRequiredLevels?.[refName]
        : null) ?? null,
    [refType, refName, libraryIndex]
  )

  const effectiveAcquiredLevel =
    acquiredLevelOverride != null ? acquiredLevelOverride : castableAcquiredLevel
  ...
  const handleRefNameChange = async (name) => {
    ...
    if (refType === 'castable' && name && activeLibrary) {
      try {
        ...
        /* castable not found, leave fields as-is */
      }
    }
    if (refType === 'status') {
          ...
          <Box sx={{ display: 'flex', gap: 2, alignItems: 'flex-start', flexWrap: 'wrap' }}>
            ...
            <Autocomplete
              ...
              renderInput={(params) => (
                <TextField
                  {...params}
                  label={refType === 'castable' ? 'Castable Name' : 'Status Name'}
                />
              )}
            />
            <FormControl size="small" sx={{ minWidth: 100 }}>
              ...
              <Select
                ...
                onChange={(e) => {
                  markDirtyLocal()
                  setSpellOrSkill(e.target.value)
                }}
              >
            ...
            <Box sx={{ display: 'flex', gap: 2, mt: 1.5, alignItems: 'center' }}>
              ...
              <FormControl size="small" sx={{ minWidth: 140 }}>
                ...
                <Select
                  ...
                  onChange={(e) => {
                    markDirtyLocal()
                    setBudgetDimension(e.target.value)
                  }}
                >
        ...
        <Accordion variant="outlined" defaultExpanded disableGutters>
          <AccordionSummary expandIcon={<ExpandMoreIcon />}>
            <Typography variant="subtitle2">
              Formula builder — coefficient · base · weapon · stats · constants
            </Typography>
          </AccordionSummary>
          <AccordionDetails>
              ...
              <Box>
                ...
                <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap' }}>
                  ...
                  <FormControl size="small" sx={{ minWidth: 130 }}>
                    ...
                    <Select
                      value={coeffTargeting}
                      label="Targeting"
                      onChange={(e) => {
                        markDirtyLocal()
                        setCoeffTargeting(e.target.value)
                      }}
                    >
                  ...
                  {coeffTargeting !== 'ASSAIL' && (
                    <FormControl size="small" sx={{ minWidth: 200 }}>
                      ...
                      <Select
                        ...
                        onChange={(e) => {
                          markDirtyLocal()
                          setCoeffDelivery(e.target.value)
                        }}
                      >
                  ...
                  return [
                    ...
                    (baseParam || randParam) && (
                      <Box key="base">
                        ...
                        >
                          {baseParam && (
                            <NumberParam
                              value={paramValues[baseParam.key]}
                              label="Base Damage"
                          ...
                          {randParam &&
                            (() => {
                              const rv = paramValues[randParam.key] || {
                                variable: '',
                                multiplier: 1
                              }
                              return (
                        ...
                        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                            ...
                            control={
                              <Checkbox
                                ...
                                onChange={(e) => {
                                  ...
                                  else if (!paramValues[weaponParam.key])
                                    updateParam(weaponParam.key, 1)
                                }}
                              />
                            }
                            label={
                              <Typography variant="body2">
                                Enable weapon damage (SOURCEWEAPONSMALLDAMAGE)
                              </Typography>
                            }
                          />
                          {(paramValues._weaponEnabled ?? false) && (
                            <>
                              ...
                              <NumberParam
                                value={paramValues[weaponParam.key]}
                                label="Coefficient"
                    ...
                    costParam && (
                      <Box key="cost">
                        ...
                        <NumberParam
                          value={paramValues[costParam.key]}
                          label="Mana Cost"
