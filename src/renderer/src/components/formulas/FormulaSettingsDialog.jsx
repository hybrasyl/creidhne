...
const MODE_LABELS = {
  none: 'None',
  linearStep: 'Linear Step',
  binary: 'Binary',
  steppedTiers: 'Stepped Tiers'
}
        ...
        <Box sx={{ display: 'flex', gap: 2, mb: 2 }}>
          ...
          {bm.mode === 'linearStep' && (
            <FormControl size="small" sx={{ minWidth: 160 }}>
              ...
              <Select
                value={bm.application}
                label="Application"
                onChange={(e) => setBM('application', e.target.value)}
              >
                {BUDGET_APPLICATIONS.map((a) => (
              ...
              <TableRow key={fc.key}>
                ...
                <TableCell>
                  <TextField
                    ...
                    sx={{ width: 100 }}
                    value={vars[fc.key] ?? ''}
                    placeholder={String(fc.default)}
                    onChange={(e) =>
                      setVar(fc.key, e.target.value === '' ? '' : Number(e.target.value))
                    }
                  />
                </TableCell>
                <TableCell>
      ...
      <Box>
        ...
        {Object.keys(vars).filter((k) => !FORMULA_CONSTANT_KEYS.includes(k)).length > 0 && (
          <Table size="small" sx={{ mb: 1 }}>
            ...
            <TableBody>
              {Object.entries(vars)
                .filter(([k]) => !FORMULA_CONSTANT_KEYS.includes(k))
                .map(([key, val]) => (
                  <TableRow key={key}>
                    <TableCell>
                      <Typography variant="body2" sx={{ fontFamily: 'monospace' }}>
                    ...
                    <TableCell>
                      <TextField
                        ...
                        value={val}
                        onChange={(e) =>
                          setVar(key, e.target.value === '' ? '' : Number(e.target.value))
                        }
                      />
                    </TableCell>
                    <TableCell>
                      <IconButton size="small" onClick={() => deleteVar(key)}>
