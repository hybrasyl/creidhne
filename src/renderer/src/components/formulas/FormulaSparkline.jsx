...
export default function FormulaSparkline({ lines, width = 240, height = 80, xLabel }) {
  ...
  const { svgLines, xRange, yRange } = useMemo(() => {
    ...
    if (yMin === yMax) { yMin -= 1; yMax += 1; } // avoid zero-height
    ...
    const svg = lines.map((l) => ({
      label: l.label,
      color: l.color,
      d: l.points.map(([x, y], i) => `${i === 0 ? 'M' : 'L'} ${toX(x).toFixed(2)} ${toY(y).toFixed(2)}`).join(' '),
    }));
    ...
    <Box sx={{ width, p: 1 }}>
      <svg viewBox={`0 0 ${vbW} ${vbH}`} width={width} height={height} style={{ display: 'block' }}>
        ...
        <line x1={pad.l} y1={vbH - pad.b} x2={vbW - pad.r} y2={vbH - pad.b}
          stroke="currentColor" strokeOpacity="0.15" strokeWidth="0.3" />
        {svgLines.map((l, i) => (
          <path key={i} d={l.d} fill="none" stroke={l.color} strokeWidth="0.8" strokeLinejoin="round" strokeLinecap="round" />
        ))}
      </svg>
