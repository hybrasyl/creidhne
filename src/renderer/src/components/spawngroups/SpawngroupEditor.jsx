      ...
      {loot.map((entry, i) => (
        ...
        >
          <Autocomplete
            ...
            value={entry.name}
            onInputChange={(_, val, reason) => {
              if (reason === 'input') set(i, 'name', val)
            }}
      ...
      {[
        {
          key: 'players',
          label: 'Players',
          exceptKey: 'playerExceptCookie',
          onlyKey: 'playerOnlyCookie'
        },
        {
          key: 'monsters',
          label: 'Monsters',
          exceptKey: 'monsterExceptCookie',
          onlyKey: 'monsterOnlyCookie'
        }
        ...
        <Box sx={{ p: 2, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
          ...
          <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
            ...
            <Autocomplete
              ...
              value={spawn.import}
              onInputChange={(_, val, reason) => {
                if (reason === 'input') set('import', val)
              }}
          ...
          >
            {spawn.immunities.map((row, i) => (
              <ImmunityRow
                ...
                onChangeField={(idx, field, val) =>
                  set(
                    'immunities',
                    spawn.immunities.map((imm, j) => (j === idx ? { ...imm, [field]: val } : imm))
                  )
                }
                onChangeType={(idx, newType) =>
                  set(
                    'immunities',
                    spawn.immunities.map((imm, j) =>
                      j === idx ? { ...imm, type: newType, value: '' } : imm
                    )
                  )
                }
                onRemove={(idx) =>
                  set(
                    'immunities',
                    spawn.immunities.filter((_, j) => j !== idx)
                  )
                }
              />
            ))}
            <Button
              ...
              onClick={() =>
                set('immunities', [
                  ...spawn.immunities,
                  { type: 'Element', value: '', messageType: 'Say', message: '' }
                ])
              }
            >
              Add Immunity
          ...
          <Section
            ...
            onToggle={() => setOpenCoordinates((v) => !v)}
          >
            {spawn.coordinates.map((coord, i) => (
              <Box key={i} sx={{ display: 'flex', gap: 1, alignItems: 'center', mb: 1 }}>
                <TextField
                  ...
                  value={coord.x}
                  onChange={(e) => {
                    const updated = spawn.coordinates.map((c, j) =>
                      j === i ? { ...c, x: negInt(e.target.value) } : c
                    )
                ...
                <TextField
                  ...
                  onChange={(e) => {
                    const updated = spawn.coordinates.map((c, j) =>
                      j === i ? { ...c, y: negInt(e.target.value) } : c
                    )
                    set('coordinates', updated)
                  }}
                  slotProps={{
                ...
                <IconButton
                  ...
                  onClick={() =>
                    set(
                      'coordinates',
                      spawn.coordinates.filter((_, j) => j !== i)
                    )
                  }
                >
                  <DeleteIcon fontSize="small" />
          ...
          <Section title="Spawn Base" open={openBase} onToggle={() => setOpenBase((v) => !v)}>
            <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
              <Autocomplete
                ...
                onInputChange={(_, val, reason) => {
                  if (reason === 'input') setNested('base', 'behaviorSet', val)
                }}
          ...
          >
            <CookiesContent cookies={spawn.cookies} onChange={(val) => set('cookies', val)} />
          </Section>
        </Box>
      </Collapse>
    </Paper>
...
function SpawngroupEditor({
  ...
  const [prefix, setPrefix] = useState(() =>
    deriveSpawngroupPrefix(initialFileName, spawngroup.name)
  )
