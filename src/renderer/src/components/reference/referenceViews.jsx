...
function Row({ label, children }) {
  ...
  return (
    <Box sx={{ display: 'flex', gap: 1, py: 0.25 }}>
      <Typography variant="caption" sx={{ minWidth: 100, color: 'text.secondary', textTransform: 'uppercase', letterSpacing: 0.5, pt: 0.25 }}>
        {label}
      </Typography>
...
function ChipList({ items }) {
  ...
  return (
    <Stack direction="row" spacing={0.5} flexWrap="wrap" useFlexGap>
      {items.map((v) => <Chip key={v} label={v} size="small" variant="outlined" />)}
    </Stack>
  );
}
...
function SectionHeading({ children }) {
  return (
    <Typography variant="overline" sx={{ display: 'block', mt: 2, mb: 0.5, color: 'text.button', fontWeight: 'bold' }}>
      {children}
    </Typography>
  );
}
...
export function CastableReferenceView({ data }) {
  ...
  return (
    <Box>
      ...
      <Row label="Classes"><ChipList items={classes} /></Row>
      ...
      <Row label="Categories"><ChipList items={categories} /></Row>
      ...
      {descriptions.length > 0 && (
        <>
          ...
          {descriptions.map((d, i) => (
            <Box key={i} sx={{ mb: 1 }}>
              <Typography variant="caption" color="text.secondary">{d.class || 'All'}</Typography>
      ...
      {data.script ? (
        <>
          ...
          <Typography variant="body2" sx={{ fontFamily: 'monospace' }}>{data.script}</Typography>
        </>
      ) : null}
    </Box>
  );
...
export function StatusReferenceView({ data }) {
  ...
  return (
    <Box>
      <Typography variant="h6" gutterBottom noWrap>{data.name}</Typography>

      <Row label="Icon">{data.icon}</Row>
      <Row label="Duration">{data.duration ? `${data.duration} s` : null}</Row>
      <Row label="Tick">{data.tick ? `${data.tick} s` : null}</Row>
      <Row label="Categories"><ChipList items={categories} /></Row>
      <Row label="Flags"><ChipList items={flags} /></Row>
        ...
        <>
          ...
          {loot.map((entry, i) => (
            <Box key={i} sx={{ display: 'flex', gap: 1, py: 0.25 }}>
              <Typography variant="body2" sx={{ flex: 1, minWidth: 0 }} noWrap>{entry.name}</Typography>
              <Typography variant="caption" color="text.secondary">{entry.rolls} × {entry.chance}</Typography>
            </Box>
          ))}
        </>
      )}
...
export function ReferenceViewDispatcher({ type, parsed, raw, showXml }) {
  ...
  switch (type) {
    case 'castables': return <CastableReferenceView data={parsed} />;
    case 'statuses':  return <StatusReferenceView data={parsed} />;
    case 'items':     return <ItemReferenceView data={parsed} />;
    case 'creatures': return <CreatureReferenceView data={parsed} />;
    default:          return <RawXmlView raw={raw} />;
  }
}
