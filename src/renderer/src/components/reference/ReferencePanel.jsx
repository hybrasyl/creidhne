...
export default function ReferencePanel() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    window.electronAPI
      .loadReference(activeLibrary, type, name)
      .then((r) => {
        if (!cancelled) setResult(r)
      })
      .catch((err) => {
        if (!cancelled) setResult({ ok: false, error: err?.message || String(err) })
      })
      .finally(() => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Typography
          variant="subtitle2"
          sx={{ flex: 1, fontWeight: 'bold', letterSpacing: 1, textTransform: 'uppercase' }}
        >
          Reference
        </Typography>
      ...
      <Box sx={{ flex: 1, overflow: 'auto', p: 1.5, minHeight: 0 }}>
        {!activeLibrary && (
          <Alert severity="info" variant="outlined">
            Select an active library to use the reference panel.
          </Alert>
        )}
        {activeLibrary && !name && (
        ...
        {!loading && result && !result.ok && <Alert severity="warning">{result.error}</Alert>}
        {!loading && result?.ok && (
