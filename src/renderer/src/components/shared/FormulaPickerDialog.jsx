...
import {
  ...
  Button, TextField, InputAdornment, List, ListItemButton, ListItemText,
    ...
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent dividers sx={{ display: 'flex', flexDirection: 'column', gap: 1, p: 1.5 }}>
        <Box sx={{ display: 'flex', gap: 1 }}>
          <TextField
            size="small" placeholder="Filter by name..." sx={{ flex: 1 }}
            value={search} onChange={(e) => setSearch(e.target.value)}
            InputProps={{ startAdornment: <InputAdornment position="start"><SearchIcon fontSize="small" /></InputAdornment> }}
          />
          <TextField
            size="small" select label="Category" sx={{ width: 130 }}
            value={catFilter} onChange={(e) => { setCatFilter(e.target.value); setSelected(null); }}
        ...
        {loading ? (
          ...
          <Typography variant="body2" color="text.secondary" sx={{ p: 2 }}>No formulas found.</Typography>
        ) : (
          <List dense sx={{ maxHeight: 320, overflow: 'auto' }}>
            {filtered.map((f) => (
              <ListItemButton key={f.id} selected={selected?.id === f.id} onClick={() => setSelected(f)}
                onDoubleClick={() => { setSelected(f); onSelect(f); onClose(); }}>
                <ListItemText
                  primary={
      ...
      <DialogActions>
        ...
        <Button variant="contained" disabled={!selected} onClick={handleConfirm}>Select</Button>
      </DialogActions>
    </Dialog>
  );
}
