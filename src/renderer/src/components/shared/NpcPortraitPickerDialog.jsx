  ...
  return (
    <div style={style} onClick={() => onSelect(name)}>
      <Box sx={{
        ...
        <Typography sx={{ fontSize: '0.72rem', color: 'text.secondary', lineHeight: 1, textAlign: 'center', px: 0.5, overflow: 'hidden', textOverflow: 'ellipsis', width: '100%' }}>
          {name}
        </Typography>
      </Box>
    </div>
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth={false} PaperProps={{ sx: { overflowX: 'hidden' } }}>
      ...
      <DialogContent sx={{ p: 1, pt: '8px !important', overflowX: 'hidden', width: COLS * CELL_SIZE + 33 }}>
        <TextField
          size="small" fullWidth placeholder="Filter by name..."
