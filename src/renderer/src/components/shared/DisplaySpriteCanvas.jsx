 ...
 */
export default function DisplaySpriteCanvas({ category, gender, displaySprite, size = 96, label, pose, frameIdx }) {
  ...
  useEffect(() => {
    ...
    if (!clientPath || !category || !gender || displaySprite == null || String(displaySprite) === '') return undefined;
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0.25 }}>
      ...
      {label && (
        <Typography sx={{ fontSize: '0.7rem', color: 'text.secondary', lineHeight: 1 }}>{label}</Typography>
      )}
    </Box>
  );
}
