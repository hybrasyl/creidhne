...
import {
  ...
  IconButton, InputAdornment, CircularProgress, ToggleButton, ToggleButtonGroup,
  ...
  return (
    <div style={style} onClick={() => onSelect(id)}>
      <Box sx={{
        ...
        <DisplaySpriteCanvas category={category} gender={gender} displaySprite={id} size={IMAGE_SIZE} pose={pose} frameIdx={frameIdx} />
        <Typography sx={{ fontSize: '0.75rem', color: 'text.secondary', lineHeight: 1 }}>{id}</Typography>
      </Box>
    </div>
  );
}
...
export default function DisplaySpritePickerDialog({ open, category, value, onClose, onChange }) {
  ...
  const catDefaults = CATEGORY_DEFAULTS[String(firstCategory || '').toUpperCase()] || DIALOG_GLOBAL_DEFAULT;
      ...
      <DialogTitle sx={{ display: 'flex', alignItems: 'center', py: 1.6 }}>
        ...
        {category && (
          <Typography variant="caption" sx={{ ml: 1.5, color: 'text.secondary' }}>
            (category {Array.isArray(category) ? category.join('+') : category}{ids ? `, ${ids.length.toLocaleString()} ids` : ''})
          </Typography>
        )}
        <ToggleButtonGroup
          size="small" exclusive value={gender} onChange={(_, v) => v && setGender(v)}
          sx={{ ml: 2 }}
        >
        ...
        <FormControl size="small" sx={{ ml: 1.5, minWidth: 88 }}>
          ...
          <Select label="Pose" value={pose} onChange={(e) => setPose(e.target.value)}>
            {POSES.map((p) => <MenuItem key={p} value={p}>{p}</MenuItem>)}
          </Select>
        </FormControl>
        <FormControl size="small" sx={{ ml: 1, minWidth: 92 }}>
          <InputLabel>Frame</InputLabel>
          <Select label="Frame" value={frameIdx} onChange={(e) => setFrameIdx(Number(e.target.value))}>
            {FRAME_RANGE.map((n) => <MenuItem key={n} value={n}>{n}</MenuItem>)}
          </Select>
        </FormControl>
