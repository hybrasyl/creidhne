...
import {
  ...
  TextField, Box, Typography, IconButton, InputAdornment,
} from '@mui/material';
...
function SpriteCell({ columnIndex, rowIndex, style, data }) {
  ...
  return (
    <div style={style} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} onClick={() => onSelect(key)}>
      <Box
        sx={{
      ...
      >
        <Box sx={{ width: IMAGE_SIZE, height: IMAGE_SIZE, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
...
export default function SpritePickerDialog({ open, value, onClose, onChange }) {
  ...
  useEffect(() => {
    ...
    if (idx < 0) return;
    gridRef.current.scrollToItem({ columnIndex: idx % COLS, rowIndex: Math.floor(idx / COLS), align: 'smart' });
  }, [open]); // eslint-disable-line react-hooks/exhaustive-deps

  const cellData = useMemo(() => ({ entries: filtered, selectedKey, onSelect: onChange }), [filtered, selectedKey, onChange]);
