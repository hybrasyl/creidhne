...
import {
  useNationCrestIndex, getAvailableCrestPalettes, DEFAULT_CREST_PALETTE,
...
function Cell({ columnIndex, rowIndex, style, data }) {
  const { ids, selectedId, onSelect, paletteOverride } = data;
    ...
    getAvailableCrestPalettes(clientPath)
      .then((list) => {
        ...
        if (paletteKey == null && list.length) {
          const def = list.find((p) =>
            p.pattern === DEFAULT_CREST_PALETTE.pattern && p.number === DEFAULT_CREST_PALETTE.number
          );
