...
import { ALL_CASTABLE_CLASSES, isAllClasses, DEFAULT_REQUIREMENT } from '../../data/castableConstants';

const STAT_FIELDS = [
  { key: 'str', label: 'Str' }, { key: 'int', label: 'Int' },
  { key: 'wis', label: 'Wis' }, { key: 'con', label: 'Con' },
  { key: 'dex', label: 'Dex' },
];

function RequirementAccordion({ requirement: req, castableNames, itemNames, npcStringKeys, onChange, onRemove }) {
  const [open, setOpen] = useState(true);

  const set        = (field, val) => onChange({ ...req, [field]: val });
  const setNumeric = (field)      => (e) => set(field, e.target.value.replace(/\D/g, ''));
  const setPr      = (field, val) => onChange({ ...req, prerequisites: { ...req.prerequisites, [field]: val } });

  const displayClass = isAllClasses(req.class) ? 'All' : (req.class || '(any)');

  const addItem    = ()     => set('items', [...(req.items || []), { itemName: '', quantity: '1' }]);
  const updateItem = (i, v) => set('items', (req.items || []).map((x, idx) => idx === i ? v : x));
  const removeItem = (i)    => set('items', (req.items || []).filter((_, idx) => idx !== i));

  const addCastable    = ()     => setPr('castables', [...(req.prerequisites?.castables || []), { name: '', level: '' }]);
  const updateCastable = (i, v) => setPr('castables', (req.prerequisites?.castables || []).map((x, idx) => idx === i ? v : x));
  const removeCastable = (i)    => setPr('castables', (req.prerequisites?.castables || []).filter((_, idx) => idx !== i));
  ...
  return (
    <Paper variant="outlined" sx={{ mb: 1.5 }}>
      ...
      >
        ...
        <IconButton size="small" color="error" onClick={(e) => { e.stopPropagation(); onRemove(); }} sx={{ mr: 0.5 }}>
        ...
        <Box sx={{ p: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', alignItems: 'flex-start' }}>
            {STAT_FIELDS.map(({ key, label }) => (
              <TextField
                key={key} label={label} size="small" sx={{ width: 80 }}
                value={req[key] || ''} onChange={setNumeric(key)}
          ...
          <Paper variant="outlined" sx={{ p: 1.5 }}>
            <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 1 }}>Prerequisites</Typography>
            <FormControlLabel
              control={
                <Checkbox size="small" checked={!!req.prerequisites?.deprecated}
            ...
            {(req.prerequisites?.castables || []).map((c, i) => (
              <Box key={i} sx={{ display: 'flex', gap: 1, alignItems: 'center', mb: 1 }}>
                ...
                />
                <TextField
                  label="Level" size="small" sx={{ width: 80 }}
                  value={c.level || ''}
                  onChange={(e) => updateCastable(i, { ...c, level: e.target.value.replace(/\D/g, '') })}
            ...
            <Button size="small" startIcon={<AddIcon />} onClick={addCastable}>Add Castable</Button>
          </Paper>

          {/* Required Items sub-section */}
          <Paper variant="outlined" sx={{ p: 1.5 }}>
            <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 1 }}>Required Items</Typography>
            {(req.items || []).map((item, i) => (
              <Box key={i} sx={{ display: 'flex', gap: 1, alignItems: 'center', mb: 1 }}>
                ...
                <TextField
                  ...
                  value={item.quantity || ''}
                  onChange={(e) => updateItem(i, { ...item, quantity: e.target.value.replace(/\D/g, '') })}
                  inputProps={{ inputMode: 'numeric' }}
                />
...
function RequirementsSection({ requirements, libraryIndex, onChange }) {
  ...
  return (
    <Box>
      ...
      <Button size="small" startIcon={<AddIcon />} onClick={add}>Add Requirement</Button>
    </Box>
  );
}
