...
function AddStatusRow({ entry, statusNames, onChange, onRemove }) {
  ...
  return (
    <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', mb: 1, flexWrap: 'wrap' }}>
      <Typography variant="caption" sx={{ width: 46, color: 'text.secondary', flexShrink: 0 }}>
        Add
      </Typography>
      <Autocomplete
        freeSolo
      ...
      <TextField
        ...
        onChange={(e) => set('intensity', e.target.value.replace(/[^\d.]/g, ''))}
      />
      <TextField
        label="Tick"
        size="small"
...
function StatusesSection({ statuses, libraryIndex, onChange }) {
  ...
  const updateAdd = (i, v) =>
    onChange({ ...statuses, add: add.map((x, idx) => (idx === i ? v : x)) })
  const deleteAdd = (i) => onChange({ ...statuses, add: add.filter((_, idx) => idx !== i) })
  const updateRemove = (i, v) =>
    onChange({ ...statuses, remove: remove.map((x, idx) => (idx === i ? v : x)) })
  const deleteRemove = (i) =>
    onChange({ ...statuses, remove: remove.filter((_, idx) => idx !== i) })
