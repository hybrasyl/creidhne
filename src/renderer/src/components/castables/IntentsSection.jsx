...
import {
  SPELL_USE_TYPES, INTENT_FLAGS, INTENT_DIRECTIONS, VISUAL_EFFECT_TYPES, DEFAULT_INTENT,
  ...
  return (
    <FormControl size="small" sx={{ width: 110 }}>
      ...
      <Select value={value || 'Targets'} label="Visual" onChange={(e) => onChange(e.target.value)}>
        {VISUAL_EFFECT_TYPES.map((v) => <MenuItem key={v} value={v}>{v}</MenuItem>)}
      </Select>
    </FormControl>
  );
}
  ...
  return (
    <FormControl size="small" sx={{ width: 110 }}>
      ...
      <Select value={value || 'None'} label="Direction" onChange={(e) => onChange(e.target.value)}>
        {INTENT_DIRECTIONS.map((d) => <MenuItem key={d} value={d}>{d}</MenuItem>)}
      </Select>
    </FormControl>
  );
}
...
function CrossRow({ shape, onChange, onRemove }) {
  return (
    <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', mb: 1, flexWrap: 'wrap' }}>
      ...
      <TextField label="Radius" size="small" sx={{ width: 100 }}
        value={shape.radius} onChange={(e) => onChange({ ...shape, radius: e.target.value })}
        inputProps={{ inputMode: 'numeric' }}
      />
      <VisualEffectSelect value={shape.visualEffect} onChange={(v) => onChange({ ...shape, visualEffect: v })} />
  ...
  return (
    <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', mb: 1, flexWrap: 'wrap' }}>
      ...
      <TextField label="Radius" size="small" sx={{ width: 100 }}
        ...
        inputProps={{ inputMode: 'numeric' }}
      />
      <DirectionSelect value={shape.direction} onChange={(v) => onChange({ ...shape, direction: v })} />
      <VisualEffectSelect value={shape.visualEffect} onChange={(v) => onChange({ ...shape, visualEffect: v })} />
      <IconButton size="small" color="error" onClick={onRemove}><DeleteIcon fontSize="small" /></IconButton>
    </Box>
  );
...
function SquareRow({ shape, onChange, onRemove }) {
  return (
    <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', mb: 1, flexWrap: 'wrap' }}>
      ...
      <TextField label="Side" size="small" sx={{ width: 100 }}
        value={shape.side} onChange={(e) => onChange({ ...shape, side: e.target.value })}
        inputProps={{ inputMode: 'numeric' }}
      />
      <VisualEffectSelect value={shape.visualEffect} onChange={(v) => onChange({ ...shape, visualEffect: v })} />
      <IconButton size="small" color="error" onClick={onRemove}><DeleteIcon fontSize="small" /></IconButton>
...
function LineRow({ shape, onChange, onRemove }) {
  return (
    <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', mb: 1, flexWrap: 'wrap' }}>
      ...
      <TextField label="Length" size="small" sx={{ width: 100 }}
        value={shape.length} onChange={(e) => onChange({ ...shape, length: e.target.value })}
        inputProps={{ inputMode: 'numeric' }}
      />
      <DirectionSelect value={shape.direction} onChange={(v) => onChange({ ...shape, direction: v })} />
      <VisualEffectSelect value={shape.visualEffect} onChange={(v) => onChange({ ...shape, visualEffect: v })} />
...
function TileRow({ shape, onChange, onRemove }) {
  return (
    <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', mb: 1, flexWrap: 'wrap' }}>
      ...
      <DirectionSelect value={shape.direction} onChange={(v) => onChange({ ...shape, direction: v })} />
      <TextField label="Rel X" size="small" sx={{ width: 80 }}
        value={shape.relativeX} onChange={(e) => onChange({ ...shape, relativeX: e.target.value })}
        inputProps={{ inputMode: 'numeric' }}
      />
      <TextField label="Rel Y" size="small" sx={{ width: 80 }}
        value={shape.relativeY} onChange={(e) => onChange({ ...shape, relativeY: e.target.value })}
        inputProps={{ inputMode: 'numeric' }}
      />
      <VisualEffectSelect value={shape.visualEffect} onChange={(v) => onChange({ ...shape, visualEffect: v })} />
      <IconButton size="small" color="error" onClick={onRemove}><DeleteIcon fontSize="small" /></IconButton>
...
function IntentItem({ intent, index, onChange, onRemove }) {
  ...
  const updateShape = (key, i, val)   => set(key, intent[key].map((s, idx) => idx === i ? val : s));
  const removeShape = (key, i)        => set(key, intent[key].filter((_, idx) => idx !== i));
    ...
    <Paper variant="outlined" sx={{ mb: 1.5 }}>
      ...
      >
        <Typography variant="body2" sx={{ fontWeight: 500, flex: 1 }}>
          Intent {index + 1}{intent.useType ? ` — ${intent.useType}` : ''}
        </Typography>
        ...
        {open ? <ExpandLessIcon fontSize="small" sx={{ ml: 0.5 }} /> : <ExpandMoreIcon fontSize="small" sx={{ ml: 0.5 }} />}
      </Box>
        ...
        <Box sx={{ p: 2, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
          ...
          <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
            <FormControl size="small" sx={{ minWidth: 150 }}>
              ...
              <Select value={intent.useType || 'NoTarget'} label="Use Type"
                onChange={(e) => set('useType', e.target.value)}
              >
                {SPELL_USE_TYPES.map((t) => <MenuItem key={t} value={t}>{t}</MenuItem>)}
              </Select>
            </FormControl>
            <TextField
              label="Max Targets" size="small" sx={{ width: 120 }}
          ...
          <Box>
            ...
            >
              ...
              <FormControlLabel
                control={
                  <Checkbox size="small" checked={intent.map}
              ...
              <Box sx={{ display: 'flex', gap: 0.75, flexWrap: 'wrap', mb: 1.5 }}>
                <Button size="small" startIcon={<AddIcon />} onClick={() => addShape('crosses', DEFAULT_CROSS)}>Cross</Button>
                <Button size="small" startIcon={<AddIcon />} onClick={() => addShape('cones',   DEFAULT_CONE)}>Cone</Button>
                <Button size="small" startIcon={<AddIcon />} onClick={() => addShape('squares', DEFAULT_SQUARE)}>Square</Button>
                <Button size="small" startIcon={<AddIcon />} onClick={() => addShape('lines',   DEFAULT_LINE)}>Line</Button>
                <Button size="small" startIcon={<AddIcon />} onClick={() => addShape('tiles',   DEFAULT_TILE)}>Tile</Button>
              </Box>

              {/* Shape rows */}
              {intent.crosses?.map((s, i) => (
                <CrossRow  key={`cross-${i}`}  shape={s} onChange={(v) => updateShape('crosses', i, v)} onRemove={() => removeShape('crosses', i)} />
              ))}
              {intent.cones?.map((s, i) => (
                <ConeRow   key={`cone-${i}`}   shape={s} onChange={(v) => updateShape('cones',   i, v)} onRemove={() => removeShape('cones',   i)} />
              ))}
              {intent.squares?.map((s, i) => (
                <SquareRow key={`square-${i}`} shape={s} onChange={(v) => updateShape('squares', i, v)} onRemove={() => removeShape('squares', i)} />
              ))}
              {intent.lines?.map((s, i) => (
                <LineRow   key={`line-${i}`}   shape={s} onChange={(v) => updateShape('lines',   i, v)} onRemove={() => removeShape('lines',   i)} />
              ))}
              {intent.tiles?.map((s, i) => (
                <TileRow   key={`tile-${i}`}   shape={s} onChange={(v) => updateShape('tiles',   i, v)} onRemove={() => removeShape('tiles',   i)} />
              ))}

              {/* Empty state */}
              {!intent.map &&
               !intent.crosses?.length && !intent.cones?.length && !intent.squares?.length &&
               !intent.lines?.length   && !intent.tiles?.length && (
