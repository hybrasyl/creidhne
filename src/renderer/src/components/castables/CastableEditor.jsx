...
import {
  CASTABLE_BOOKS,
  CASTABLE_DESCRIPTION_CLASSES,
  CASTABLE_COST_TYPES,
  ALL_CASTABLE_CLASSES,
  computeCastableFilename,
  derivePrefix,
  MASTERY_MODIFIERS,
  BOOK_ABBREVS
...
function CastableEditor({
  ...
  const setMeta = (field) => (e) =>
    updateData((d) => ({ ...d, meta: { ...d.meta, [field]: e.target.checked } }))
  const setMetaText = (field) => (e) =>
    updateData((d) => ({ ...d, meta: { ...d.meta, [field]: e.target.value } }))
  ...
  const setDescription = (i, field, val) =>
    updateData((d) => ({
      ...d,
      descriptions: d.descriptions.map((x, idx) => (idx === i ? { ...x, [field]: val } : x))
    }))
  ...
  const addCastCost = () =>
    updateData((d) => ({
      ...d,
      castCosts: [...d.castCosts, { type: 'Hp', value: '', quantity: '1', itemName: '' }]
    }))
  const setCastCost = (i, field, val) =>
    updateData((d) => ({
      ...d,
      castCosts: d.castCosts.map((c, idx) => (idx === i ? { ...c, [field]: val } : c))
    }))
  const setCastCostType = (i, newType) =>
    updateData((d) => ({
      ...d,
      castCosts: d.castCosts.map((c, idx) =>
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <Box sx={{ flex: 1, overflow: 'auto' }}>
              ...
              <FormControl size="small" sx={{ minWidth: 180 }}>
                ...
                <Select
                  ...
                  onChange={(e) => {
                    ...
                    if (!prefixEdited) {
                      const newPrefix =
                        deriveMetaPrefix(data.meta, newBook) ?? derivePrefix(data.class, newBook)
                      setPrefix(newPrefix)
                      if (!fileNameEdited)
                        setFileName(computeCastableFilename(newPrefix, data.name))
                    }
                    updateData((d) => ({ ...d, book: newBook }))
                  }}
                >
                  {CASTABLE_BOOKS.map((b) => (
            ...
            <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
              <FormControlLabel
                control={
                  <Checkbox
                    size="small"
                    checked={data.isAssail}
                    onChange={setChecked('isAssail')}
                  />
                }
              ...
              <FormControlLabel
                control={
                  <Checkbox
                    size="small"
                    checked={data.reflectable}
                    onChange={setChecked('reflectable')}
                  />
                }
              ...
              <FormControlLabel
                control={
                  <Checkbox
                    size="small"
                    checked={data.breakStealth}
                    onChange={setChecked('breakStealth')}
                  />
                }
              ...
              <FormControlLabel
                control={
                  <Checkbox
                    ...
                    onChange={setChecked('includeInMetafile')}
                  />
                }
              ...
              <FormControlLabel
                control={
                  <Checkbox size="small" checked={data.pvpOnly} onChange={setChecked('pvpOnly')} />
                }
              ...
              <FormControlLabel
                control={
                  <Checkbox
                    size="small"
                    checked={data.scriptOverride}
                    onChange={setChecked('scriptOverride')}
                  />
                }
            ...
            <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'center' }}>
              <FormControlLabel
                control={
                  <Checkbox
                    size="small"
                    checked={!!data.meta?.isTest}
                    onChange={handleMetaOverrideChange('isTest')}
                  />
                }
              ...
              <FormControlLabel
                control={
                  <Checkbox
                    size="small"
                    checked={!!data.meta?.isGM}
                    onChange={handleMetaOverrideChange('isGM')}
                  />
                }
              ...
              <FormControlLabel
                control={
                  <Checkbox
                    size="small"
                    checked={!!data.meta?.deprecated}
                    onChange={handleMetaOverrideChange('deprecated')}
                  />
                }
              ...
              <FormControlLabel
                control={
                  <Checkbox
                    size="small"
                    checked={!!data.meta?.givenViaScript}
                    onChange={setMeta('givenViaScript')}
                  />
                }
              ...
              <TextField
                ...
                size="small"
                sx={{ width: 180 }}
                value={data.meta?.specialty || ''}
                onChange={setMetaText('specialty')}
        ...
        <Section
          ...
          onToggle={() => setOpenDescriptions((v) => !v)}
        >
          {data.descriptions.map((desc, i) => (
            <Box key={i} sx={{ display: 'flex', gap: 1, alignItems: 'center', mb: 1 }}>
              ...
              <TextField
                ...
                sx={{ flex: 1 }}
                value={desc.text}
                onChange={(e) => setDescription(i, 'text', e.target.value)}
              />
                ...
                <>
                  ...
                  <Autocomplete
                    ...
                    onInputChange={(_, val, reason) => {
                      if (reason === 'input') setCastCost(i, 'itemName', val)
                    }}
        ...
        <Section
          title="Visual / Audio"
          open={openVisualAudio}
          onToggle={() => setOpenVisualAudio((v) => !v)}
        >
          <AnimationsSection
            sound={data.sound || { id: '' }}
        ...
        <Section title="Formulas" open={openFormulas} onToggle={() => setOpenFormulas((v) => !v)}>
          <FormulasSection
            ...
            onChange={({ heal: h, damage: d }) =>
              updateData((prev) => ({ ...prev, heal: h, damage: d }))
            }
          />
        </Section>
        ...
        <Section
          ...
          onToggle={() => setOpenRequirements((v) => !v)}
        >
          <RequirementsSection
