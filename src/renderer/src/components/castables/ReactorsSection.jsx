...
function ReactorRow({ reactor, index, statusNames, onChange, onRemove }) {
  ...
  const setSignedNumeric = (field) => (e) =>
    set(field, e.target.value.replace(/[^0-9-]/g, '').replace(/(?!^)-/g, ''))
      ...
      <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', flexWrap: 'wrap', pl: 1 }}>
        <FormControlLabel
          control={
            <Checkbox
              size="small"
              checked={reactor.displayOwner}
              onChange={(e) => set('displayOwner', e.target.checked)}
            />
          }
        ...
        <FormControlLabel
          control={
            <Checkbox
              size="small"
              checked={reactor.displayGroup}
              onChange={(e) => set('displayGroup', e.target.checked)}
            />
          }
        ...
        <Autocomplete
          ...
          onInputChange={(_, val, reason) => {
            if (reason === 'input') set('displayStatus', val)
          }}
