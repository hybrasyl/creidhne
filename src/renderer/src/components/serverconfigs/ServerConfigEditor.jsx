...
const DEFAULT_BOARD = {
  name: '',
  displayName: '',
  accessList: { read: [], write: [], moderate: [] }
}
...
function NetworkInfoRow({ label, value, onChange }) {
  ...
  return (
    <Box sx={{ display: 'flex', gap: 1, alignItems: 'flex-start', mb: 1.5 }}>
      <Typography variant="body2" sx={{ width: 60, mt: 1, flexShrink: 0 }}>
        {label}
      </Typography>
      <TextField
        size="small"
...
function NetworkTab({ data, updateData }) {
  ...
  const setGrpc = (field) => (e) =>
    updateData((d) => ({
      ...d,
      network: { ...d.network, grpc: { ...d.network.grpc, [field]: e.target.value } }
    }))
        ...
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
          <FormControlLabel
            control={
              <Checkbox
                size="small"
                checked={!!ds.hasCredentials}
                onChange={setDsBool('hasCredentials')}
              />
            }
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
            <Box sx={{ display: 'flex', gap: 1 }}>
              ...
              <TextField
                size="small"
                label="Port"
                value={data.network.grpc.port}
                onChange={setGrpc('port')}
                sx={{ width: 90 }}
              />
            </Box>
            <TextField
...
function AccessBoardsTab({ data, updateData }) {
  ...
  const addBoard = () =>
    updateData((d) => ({
      ...
      boards: [
        ...(d.boards || []),
        { ...DEFAULT_BOARD, accessList: { read: [], write: [], moderate: [] } }
      ]
    }))
  const removeBoard = (i) =>
  ...
  const setBoard = (i, field, val) =>
    updateData((d) => ({
      ...d,
      boards: d.boards.map((b, idx) => (idx === i ? { ...b, [field]: val } : b))
    }))

  const setBoardAl = (i, field, val) =>
    updateData((d) => ({
      ...d,
      boards: d.boards.map((b, idx) =>
        idx === i ? { ...b, accessList: { ...b.accessList, [field]: val } } : b
      )
    }))
      ...
      boards: d.boards.map((b, idx) =>
        idx === i
          ? {
              ...
              accessList: {
                ...b.accessList,
                moderate: b.accessList.moderate.map((m, midx) => (midx === mi ? val : m))
              }
            }
          : b
      )
      ...
      boards: d.boards.map((b, idx) =>
        idx === i
          ? {
              ...
              accessList: {
                ...b.accessList,
                moderate: b.accessList.moderate.filter((_, midx) => midx !== mi)
              }
            }
          : b
      )
      ...
      >
        {(data.boards || []).map((board, i) => (
          <Paper key={i} variant="outlined" sx={{ p: 2, mb: 1.5 }}>
            ...
            <Button size="small" startIcon={<AddIcon />} onClick={() => addModerate(i)}>
              Add Moderator
            </Button>
          </Paper>
        ))}
        <Button size="small" startIcon={<AddIcon />} onClick={addBoard}>
          Add Board
        </Button>
      </Section>
    </Box>
...
function TimeTab({ data, updateData }) {
  ...
  const setAge = (i, field, val) =>
    updateData((d) => ({
      ...
      time: {
        ...d.time,
        ages: d.time.ages.map((a, idx) => (idx === i ? { ...a, [field]: val } : a))
      }
    }))
  const addAge = () =>
    updateData((d) => ({
      ...d,
      time: { ...d.time, ages: [...(d.time.ages || []), { ...DEFAULT_AGE }] }
    }))
  const removeAge = (i) =>
    updateData((d) => ({
      ...d,
      time: { ...d.time, ages: d.time.ages.filter((_, idx) => idx !== i) }
    }))
  const setSs = (field) => (e) =>
    updateData((d) => ({
      ...d,
      time: { ...d.time, serverStart: { ...d.time.serverStart, [field]: e.target.value } }
    }))
  ...
  return (
    <Box>
      <Section title="Ages" open={openAges} onToggle={() => setOpenAges((v) => !v)}>
        {(data.time?.ages || []).map((age, i) => (
          <Box key={i} sx={{ display: 'flex', gap: 1, alignItems: 'flex-start', mb: 1 }}>
            ...
            <TextField
              ...
              label="Start Year"
              value={age.startYear}
              onChange={(e) => setAge(i, 'startYear', e.target.value)}
              sx={{ width: 90 }}
              placeholder="1"
            />
        ...
        <Button size="small" startIcon={<AddIcon />} onClick={addAge}>
          Add Age
        </Button>
      </Section>
      <Section title="Server Start" open={openStart} onToggle={() => setOpenStart((v) => !v)}>
        <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
          ...
          <TextField
            ...
            label="Default Year"
            sx={{ width: 110 }}
            value={data.time?.serverStart?.defaultYear ?? ''}
            onChange={setSs('defaultYear')}
          />
        </Box>
      </Section>
    </Box>
...
function HandlersTab({ data, updateData }) {
  ...
  const enableDeath = (checked) => {
    updateData((d) => ({
      ...d,
      handlers: { ...d.handlers, death: checked ? { ...DEFAULT_DEATH } : null }
    }))
  ...
  const enableChat = (checked) => {
    updateData((d) => ({
      ...
      handlers: {
        ...d.handlers,
        chat: checked ? { commandsEnabled: true, commandPrefix: '/' } : null
      }
    }))
    setOpenChat(checked)
  }
  ...
  const setDeathSub = (field, subField) => (e) =>
    updateData((d) => ({
      ...
      handlers: {
        ...
        death: {
          ...d.handlers.death,
          [field]: { ...d.handlers.death[field], [subField]: e.target.value }
        }
      }
    }))
  const setDeathBool = (field) => (e) =>
    updateData((d) => ({
      ...d,
      handlers: { ...d.handlers, death: { ...d.handlers.death, [field]: e.target.checked } }
    }))

  const setChat = (field) => (val) =>
    updateData((d) => ({
      ...d,
      handlers: { ...d.handlers, chat: { ...d.handlers.chat, [field]: val } }
    }))
    ...
    updateData((d) => ({
      ...
      handlers: {
        ...
        newPlayer: {
          ...
          startMaps: [...(d.handlers.newPlayer?.startMaps || []), { x: '', y: '', value: '' }]
        }
      }
    }))
  const setStartMap = (i, field, val) =>
    updateData((d) => ({
      ...
      handlers: {
        ...
        newPlayer: {
          ...d.handlers.newPlayer,
          startMaps: d.handlers.newPlayer.startMaps.map((sm, idx) =>
            idx === i ? { ...sm, [field]: val } : sm
          )
        }
      }
    }))
  const removeStartMap = (i) =>
    updateData((d) => ({
      ...
      handlers: {
        ...
        newPlayer: {
          ...d.handlers.newPlayer,
          startMaps: d.handlers.newPlayer.startMaps.filter((_, idx) => idx !== i)
        }
      }
    }))
  ...
  return (
    <Box>
      ...
      <Section
        title="Death"
        open={openDeath}
        onToggle={() => setOpenDeath((v) => !v)}
        enabled={!!death}
        onEnable={enableDeath}
      >
        {death && (
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            <Box sx={{ display: 'flex', gap: 2 }}>
              <FormControlLabel
                control={
                  <Checkbox size="small" checked={death.active} onChange={setDeathBool('active')} />
                }
              ...
              <FormControlLabel
                control={
                  <Checkbox
                    size="small"
                    checked={death.perishable}
                    onChange={setDeathBool('perishable')}
                  />
                }
                label={<Typography variant="body2">Perishable</Typography>}
              />
              <FormControlLabel
                control={
                  <Checkbox
                    size="small"
                    checked={death.groupNotify}
                    onChange={setDeathBool('groupNotify')}
                  />
                }
                label={<Typography variant="body2">Group Notify</Typography>}
              />
            </Box>
            <Box>
            ...
            <Box>
              ...
              <Box sx={{ display: 'flex', gap: 1, mt: 0.5 }}>
                ...
                <TextField
                  size="small"
                  label="Timeout (s)"
                  value={death.coma.timeout}
                  onChange={setDeathSub('coma', 'timeout')}
                  sx={{ width: 110 }}
                />
                <EffectPicker
                  label="Effect ID"
                  ...
                  onEffectIdChange={(val) =>
                    setDeathSub('coma', 'effect')({ target: { value: val } })
                  }
                />
              </Box>
            </Box>
            <Box>
              ...
              <Box sx={{ display: 'flex', gap: 1, mt: 0.5 }}>
                <TextField
                  size="small"
                  label="XP Loss %"
                  value={death.penalty.xp}
                  onChange={setDeathSub('penalty', 'xp')}
                  sx={{ width: 120 }}
                  placeholder="0.05"
                />
                <TextField
                  size="small"
                  label="HP Loss %"
                  value={death.penalty.hp}
                  onChange={setDeathSub('penalty', 'hp')}
                  sx={{ width: 120 }}
                  placeholder="0.05"
                />
              </Box>
            ...
            <Box>
              ...
              <Box sx={{ display: 'flex', gap: 1, mt: 0.5, alignItems: 'center' }}>
                <TextField
                  size="small"
                  label="Mark Text"
                  value={death.legendMark.value}
                  onChange={setDeathSub('legendMark', 'value')}
                  sx={{ flex: 1 }}
                />
                <TextField
                  size="small"
                ...
                <FormControlLabel
                  control={
                    <Checkbox
                      size="small"
                      checked={death.legendMark.increment}
                      onChange={(e) =>
                        updateData((d) => ({
                          ...d,
                          handlers: {
      ...
      >
        {startMaps.map((sm, i) => (
          <Box key={i} sx={{ display: 'flex', gap: 1, mb: 0.75 }}>
            ...
            <TextField
              ...
              label="X"
              value={sm.x}
              onChange={(e) => setStartMap(i, 'x', e.target.value)}
              sx={{ width: 80 }}
            />
            <TextField
              size="small"
              label="Y"
              value={sm.y}
              onChange={(e) => setStartMap(i, 'y', e.target.value)}
              sx={{ width: 80 }}
            />
        ...
        <Button size="small" startIcon={<AddIcon />} onClick={addStartMap}>
          Add Start Map
        </Button>
      </Section>
      {/* Chat */}
          ...
          <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
            <FormControlLabel
              control={
                <Checkbox
                  size="small"
                  checked={chat.commandsEnabled}
                  onChange={(e) => setChat('commandsEnabled')(e.target.checked)}
                />
              }
...
function LoggingTab({ data, updateData }) {
  ...
  const setLog = (i, field, val) =>
    updateData((d) => ({
      ...
      logging: {
        ...d.logging,
        logs: d.logging.logs.map((l, idx) => (idx === i ? { ...l, [field]: val } : l))
      }
    }))
  const addLog = () =>
    updateData((d) => ({
      ...d,
      logging: { ...d.logging, logs: [...(d.logging.logs || []), { ...DEFAULT_LOG }] }
    }))
  const removeLog = (i) =>
    updateData((d) => ({
      ...d,
      logging: { ...d.logging, logs: d.logging.logs.filter((_, idx) => idx !== i) }
    }))
  ...
  return (
    <Box>
      <Section
        title="Logging Density"
        open={openDensity}
        onToggle={() => setOpenDensity((v) => !v)}
      >
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap' }}>
          <LevelSelect
            label="Minimum Level"
            value={log.minimumLevel}
            onChange={setLogging('minimumLevel')}
            options={LOG_LEVELS}
            sx={{ width: 160 }}
          />
          <FormControlLabel
            control={
              <Checkbox
                size="small"
                checked={!!log.singleStreamEnabled}
                onChange={(e) => setLogging('singleStreamEnabled')(e.target.checked)}
              />
            }
          ...
          <FormControlLabel
            control={
              <Checkbox
                size="small"
                checked={!!log.jsonOutputEnabled}
                onChange={(e) => setLogging('jsonOutputEnabled')(e.target.checked)}
              />
            }
      ...
      <Section
        title="Logging Streams"
        open={openStreams}
        onToggle={() => setOpenStreams((v) => !v)}
      >
        {(log.logs || []).map((l, i) => (
          <Box key={i} sx={{ display: 'flex', gap: 1, mb: 1, alignItems: 'center' }}>
            <LevelSelect
              label="Type"
              value={l.type}
              onChange={(val) => setLog(i, 'type', val)}
              options={LOG_TYPES}
              sx={{ width: 160 }}
            />
            <TextField
            ...
            <LevelSelect
              label="Level"
              value={l.level}
              onChange={(val) => setLog(i, 'level', val)}
              options={LOG_LEVELS}
              sx={{ width: 130 }}
            />
        ...
        <Button size="small" startIcon={<AddIcon />} onClick={addLog}>
          Add Log Stream
        </Button>
      </Section>
    </Box>
...
function AdvancedTab({ data, updateData }) {
  ...
  const enableMetrics = (checked) =>
    updateData((d) => ({
      ...d,
      apiEndpoints: { ...d.apiEndpoints, metricsEndpoint: checked ? { url: '', apiKey: '' } : null }
    }))
  const setMetrics = (field) => (e) =>
    updateData((d) => ({
      ...
      apiEndpoints: {
        ...d.apiEndpoints,
        metricsEndpoint: { ...d.apiEndpoints.metricsEndpoint, [field]: e.target.value }
      }
    }))
  ...
  const addClientSetting = () =>
    updateData((d) => ({
      ...d,
      clientSettings: [...(d.clientSettings || []), { ...DEFAULT_CLIENT_SETTING }]
    }))
  ...
  const setCs = (i, field, val) =>
    updateData((d) => ({
      ...d,
      clientSettings: d.clientSettings.map((s, idx) => (idx === i ? { ...s, [field]: val } : s))
    }))
  ...
  const addPlugin = () =>
    updateData((d) => ({
      ...
      plugins: {
        ...
        message: [
          ...(d.plugins?.message || []),
          { ...DEFAULT_PLUGIN, configuration: [], targets: [] }
        ]
      }
    }))
  const removePlugin = (i) =>
    updateData((d) => ({
      ...d,
      plugins: { ...d.plugins, message: d.plugins.message.filter((_, idx) => idx !== i) }
    }))
  const setPlugin = (i, field, val) =>
    updateData((d) => ({
      ...
      plugins: {
        ...d.plugins,
        message: d.plugins.message.map((p, idx) => (idx === i ? { ...p, [field]: val } : p))
      }
    }))
  const addPluginConfig = (i) =>
    updateData((d) => ({
      ...
      plugins: {
        ...d.plugins,
        message: d.plugins.message.map((p, idx) =>
          idx === i ? { ...p, configuration: [...p.configuration, { key: '', value: '' }] } : p
        )
      }
    }))
      ...
      plugins: {
        ...
        message: d.plugins.message.map((p, idx) =>
          idx === i
            ? {
                ...p,
                configuration: p.configuration.map((c, cidx) =>
                  cidx === ci ? { ...c, [field]: val } : c
                )
              }
            : p
        )
  ...
  const removePluginConfig = (i, ci) =>
    updateData((d) => ({
      ...
      plugins: {
        ...d.plugins,
        message: d.plugins.message.map((p, idx) =>
          idx === i ? { ...p, configuration: p.configuration.filter((_, cidx) => cidx !== ci) } : p
        )
      }
    }))
  ...
  const setPluginTarget = (i, ti, val) =>
    updateData((d) => ({
      ...
      plugins: {
        ...d.plugins,
        message: d.plugins.message.map((p, idx) =>
          idx === i ? { ...p, targets: p.targets.map((t, tidx) => (tidx === ti ? val : t)) } : p
        )
      }
    }))
  const removePluginTarget = (i, ti) =>
    updateData((d) => ({
      ...
      plugins: {
        ...d.plugins,
        message: d.plugins.message.map((p, idx) =>
          idx === i ? { ...p, targets: p.targets.filter((_, tidx) => tidx !== ti) } : p
        )
      }
    }))
    ...
    <Box>
          ...
          <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
            <FormControlLabel
              control={
                <Checkbox
                  size="small"
                  checked={!!ep.metricsEndpoint}
                  onChange={(e) => enableMetrics(e.target.checked)}
                />
              }
      ...
      <Section title="Client Settings" open={openClient} onToggle={() => setOpenClient((v) => !v)}>
        {(data.clientSettings || []).map((s, i) => (
          <Box key={i} sx={{ display: 'flex', gap: 1, mb: 0.75, alignItems: 'center' }}>
            <TextField
              ...
              label="#"
              value={s.number}
              onChange={(e) => setCs(i, 'number', e.target.value)}
              sx={{ width: 60 }}
            />
            <TextField
              size="small"
            ...
            <TextField
              ...
              label="Label"
              value={s.value}
              onChange={(e) => setCs(i, 'value', e.target.value)}
              sx={{ flex: 1 }}
            />
            <FormControlLabel
              control={
                <Checkbox
                  size="small"
                  checked={!!s.default}
                  onChange={(e) => setCs(i, 'default', e.target.checked)}
                />
              }
        ...
        {(data.clientSettings || []).length < 8 && (
          <Button size="small" startIcon={<AddIcon />} onClick={addClientSetting}>
            Add Setting
          </Button>
        )}
      </Section>
      ...
      >
          ...
          <Paper key={i} variant="outlined" sx={{ p: 2, mb: 1.5 }}>
            <Box sx={{ display: 'flex', gap: 1, mb: 1.5, alignItems: 'center' }}>
              <LevelSelect
                label="Type"
                value={p.type}
                onChange={(val) => setPlugin(i, 'type', val)}
              ...
              <FormControlLabel
                control={
                  <Checkbox
                    size="small"
                    checked={!!p.passthrough}
                    onChange={(e) => setPlugin(i, 'passthrough', e.target.checked)}
                  />
                }
        ...
        <Button size="small" startIcon={<AddIcon />} onClick={addPlugin}>
          Add Plugin
        </Button>
      </Section>
    </Box>
