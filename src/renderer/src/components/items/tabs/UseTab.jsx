...
function UseTab({ data, onChange }) {
  ...
  const setStatuses = (updated) => onChange({ ...data, use: { ...u, statuses: updated } })
  ...
  const setAddStatus = (index, field, val) =>
    setStatuses({
      ...u.statuses,
      add: u.statuses.add.map((s, i) => (i === index ? { ...s, [field]: val } : s))
    })
  ...
  // ── Motions helpers ─────────────────────────────────────────────────────────
  const addMotion = () => onChange({ ...data, motions: [...data.motions, { id: '', speed: '' }] })
  const setMotion = (index, field, val) =>
    onChange({
      ...data,
      motions: data.motions.map((m, i) => (i === index ? { ...m, [field]: val } : m))
    })
  const removeMotion = (index) =>
    onChange({ ...data, motions: data.motions.filter((_, i) => i !== index) })

  // ── Procs helpers ───────────────────────────────────────────────────────────
  const addProc = () => onChange({ ...data, procs: [...data.procs, { ...DEFAULT_PROC }] })
  const setProc = (index, field, val) =>
    onChange({
      ...data,
      procs: data.procs.map((p, i) => (i === index ? { ...p, [field]: val } : p))
    })
  ...
  return (
    <Box>
        ...
        <>
          ...
          {u.teleport !== null && (
            <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', pl: 2, mb: 2 }}>
              <Autocomplete
                options={mapNames}
                value={u.teleport.map || null}
                onChange={(_, val) =>
                  onChange({ ...data, use: { ...u, teleport: { ...u.teleport, map: val || '' } } })
                }
                size="small"
      ...
      {data.procs.map((proc, index) => {
        ...
        const sourceType =
          proc.sourceType || (proc.castable ? 'Castable' : proc.script ? 'Script' : '')
        const switchSource = (newType) => {
          onChange({
            ...data,
            procs: data.procs.map((p, i) =>
