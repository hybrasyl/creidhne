...
describe('constantsSchema', () => {
  ...
  it('accepts mixed object-array values (cookies, motions, weapons, spellBooks)', () => {
    const r = constantsSchema.safeParse({
      cookies: [{ name: 'QuestStarted', sourceFile: '', comment: '' }],
      motions: [{ id: 1, speed: 20, name: 'Assail' }],
      weapons: [{ name: 'Steel Sword', minDmg: '5', maxDmg: '10' }]
    })
    expect(r.success).toBe(true)
  })

  it('rejects a non-array top-level value', () => {
    const r = constantsSchema.safeParse({ libraries: '/lib1' })
    expect(r.success).toBe(false)
  })
})
