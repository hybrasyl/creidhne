...
describe('castableXml — all fields', () => {
  let c
  beforeAll(async () => {
    c = await parseCastableXml(FULL_XML)
  })
  ...
  it('parses intent line shape', () => {
    expect(c.intents[0].lines[0]).toEqual({
      length: '3',
      direction: 'East',
      visualEffect: 'Targets'
    })
  })
  ...
  it('parses requirement forbidCookie', () =>
    expect(c.requirements[0].forbidCookie).toBe('notallowed'))
  it('parses requirement requireCookie', () =>
    expect(c.requirements[0].requireCookie).toBe('required'))
  ...
  // Effects — Animations
  it('parses onCast wizard motion', () =>
    expect(c.animations.onCast.player.wizard).toEqual({ id: '5', speed: '25' }))
  it('parses onCast priest motion', () =>
    expect(c.animations.onCast.player.priest).toEqual({ id: '6', speed: '20' }))
  it('parses onCast spellEffect', () =>
    expect(c.animations.onCast.spellEffect).toEqual({ id: '20', speed: '100' }))
  it('parses onCast target', () =>
    expect(c.animations.onCast.target).toEqual({ id: '10', speed: '50' }))
  it('parses onEnd spellEffect', () =>
    expect(c.animations.onEnd.spellEffect).toEqual({ id: '21', speed: '60' }))
  it('parses onEnd target', () =>
    expect(c.animations.onEnd.target).toEqual({ id: '11', speed: '30' }))
...
describe('castableXml — minimal', () => {
  let c
  beforeAll(async () => {
    c = await parseCastableXml(MINIMAL_XML)
  })
  ...
  beforeAll(async () => {
    const castable = {
      ...
      animations: {
        onCast: {
          player: {
            peasant: { id: '1', speed: '20' },
            warrior: { id: '1', speed: '20' },
            wizard: { id: '', speed: '' },
            priest: { id: '', speed: '' },
            rogue: { id: '9', speed: '15' },
            monk: { id: '', speed: '' }
