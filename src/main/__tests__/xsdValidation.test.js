...
describe('XSD validation: real-world fixtures are schema-valid', () => {
  const types = [
    'items', 'recipes', 'npcs', 'nations', 'lootsets', 'variantgroups',
    'localizations', 'creatures', 'elementtables', 'statuses', 'castables',
    'creaturebehaviorsets', 'spawngroups', 'serverconfigs',
  ]
...
describe.skip('XSD validation: serializer output is schema-valid', () => {
  it('items',           async () => { await roundtripValid(parseItemXml, serializeItemXml, await loadFixture('items')) })
  it('recipes',         async () => { await roundtripValid(parseRecipeXml, serializeRecipeXml, await loadFixture('recipes')) })
  it('npcs',            async () => { await roundtripValid(parseNpcXml, serializeNpcXml, await loadFixture('npcs')) })
  it('nations',         async () => { await roundtripValid(parseNationXml, serializeNationXml, await loadFixture('nations')) })
  it('lootsets',        async () => { await roundtripValid(parseLootXml, serializeLootXml, await loadFixture('lootsets')) })
  it('variantgroups',   async () => { await roundtripValid(parseVariantXml, serializeVariantXml, await loadFixture('variantgroups')) })
  it('localizations',   async () => { await roundtripValid(parseLocalizationXml, serializeLocalizationXml, await loadFixture('localizations')) })
  it('creatures',       async () => { await roundtripValid(parseCreatureXml, serializeCreatureXml, await loadFixture('creatures')) })
  it('element tables',  async () => { await roundtripValid(parseElementTableXml, serializeElementTableXml, await loadFixture('elementtables')) })
  it('statuses',        async () => { await roundtripValid(parseStatusXml, serializeStatusXml, await loadFixture('statuses')) })
  it('castables',       async () => { await roundtripValid(parseCastableXml, serializeCastableXml, await loadFixture('castables')) })
  it('behavior sets',   async () => { await roundtripValid(parseBehaviorSetXml, serializeBehaviorSetXml, await loadFixture('creaturebehaviorsets')) })
  it('spawn groups',    async () => { await roundtripValid(parseSpawngroupXml, serializeSpawngroupXml, await loadFixture('spawngroups')) })
  it('server configs',  async () => { await roundtripValid(parseServerConfigXml, serializeServerConfigXml, await loadFixture('serverconfigs')) })
})
