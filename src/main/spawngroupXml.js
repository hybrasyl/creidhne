...
export function parseSpawngroupXml(xmlString) {
  return new Promise((resolve, reject) => {
    ...
    xml2js.parseString(xmlString, { trim: true }, (err, result) => {
      ...
      try {
        resolve(mapXmlToSpawngroup(result, comment))
      } catch (e) {
        reject(e)
      }
    })
...
function buildSpawn(spawn) {
  ...
  const dmgAttrs = omitEmpty({
    MinDmg: spawn.combat.minDmg,
    MaxDmg: spawn.combat.maxDmg,
    Elements: spawn.combat.offensiveElement
  })
  if (Object.keys(dmgAttrs).length) node.Damage = [{ $: dmgAttrs }]

  const defAttrs = omitEmpty({
    Ac: spawn.combat.ac,
    Mr: spawn.combat.mr,
    Elements: spawn.combat.defensiveElement
  })
