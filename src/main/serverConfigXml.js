...
const CONSTANT_KEYS = [
  'MerchantBuybackPercentage', 'PlayerMaxLevel', 'ViewportSize', 'PlayerMaxBookSize',
  'PlayerMaxDropDistance', 'PlayerPickupDistance', 'PlayerExchangeDistance', 'PlayerMaxCastDistance',
  'PlayerMaxGold', 'ItemVariantIdStart', 'LogDefaultLevels', 'PlayerGroupSharingDistance',
  'PlayerAsyncDialogDistance', 'LagMap', 'NationalSpawnTimeout', 'DeathpileGroupTimeout',
  'DeathpileOtherTimeout', 'MonsterLootDropTimeout', 'MonsterTaggingTimeout',
  'ByteHeartbeatInterval', 'TickHeartbeatInterval', 'HeartbeatReaperInterval',
  'CheckpointInterval', 'RegenInterval', 'SnoreInterval', 'IdleDetectionInterval',
  'MailboxCleanupInterval', 'MerchantInventoryRefreshInterval', 'PlayerIdleCheck',
  'DialogSequenceShared', 'DialogSequencePursuits', 'DialogSequenceAsync', 'DialogSequenceHardcoded',
  'BoardMessageResponseSize', 'BoardMessageCooldown', 'MailMessageCooldown',
  'PlayerMinStat', 'PlayerMaxStat', 'PlayerMinBaseHpMp', 'PlayerMaxBaseHpMp',
  'PlayerMinDmg', 'PlayerMaxDmg', 'PlayerMinHit', 'PlayerMaxHit',
  'PlayerMinMr', 'PlayerMaxMr', 'PlayerMinAc', 'PlayerMaxAc',
  'PlayerMinRegen', 'PlayerMaxRegen',
  'ClassName0', 'ClassName1', 'ClassName2', 'ClassName3', 'ClassName4', 'ClassName5',
  'LevelCircle1', 'LevelCircle2', 'LevelCircle3', 'LevelCircle4',
];

const FORMULA_KEYS = [
  'XpToNextLevel', 'HpGainPerLevel', 'MpGainPerLevel', 'AllowedCarryWeight',
  'AllowedEquipmentWeight', 'MpRegenPerTick', 'HpRegenPerTick',
  'AcDamageMitigation', 'AcMagicDamageMitigation',
  'MonsterHpGainPerLevel', 'MonsterMpGainPerLevel',
];
...
export function parseServerConfigXml(xmlString) {
  return new Promise((resolve, reject) => {
    xml2js.parseString(xmlString, { trim: true }, (err, result) => {
      ...
      try { resolve(mapXmlToServerConfig(result)); }
      catch (e) { reject(e); }
    });
  });
}
...
function mapLogging(loggingArr) {
  ...
  if (!n) return { singleStreamEnabled: false, jsonOutputEnabled: false, minimumLevel: 'Info', logs: [] };
  return {
...
function mapDataStore(dsArr) {
  const n = first(dsArr);
  if (!n) return { type: 'redis', host: 'localhost', port: '', database: '', username: '', password: '', hasCredentials: false };
  return {
    type: a(n, 'Type', 'redis'),
...
function mapApiEndpoints(epArr) {
  const n = first(epArr);
  if (!n) return { sentry: '', encryptionEndpoint: '', validationEndpoint: '', telemetryEndpoint: '', metricsEndpoint: null };
  ...
  return {
    ...
    metricsEndpoint: metrics ? { url: a(metrics, 'Url', ''), apiKey: a(metrics, 'ApiKey', '') } : null,
  };
}
...
function mapDeath(deathArr) {
  ...
  return {
    ...
    map: mapNode
      ? { x: a(mapNode, 'X', ''), y: a(mapNode, 'Y', ''), value: typeof mapNode === 'string' ? mapNode : (mapNode._ ?? '') }
      : { x: '', y: '', value: '' },
    coma: comaNode
      ? { timeout: a(comaNode, 'Timeout', '30'), effect: a(comaNode, 'Effect', '24'), value: typeof comaNode === 'string' ? comaNode : (comaNode._ ?? '') }
  ...
  return {
    message: (msgNode?.Plugin || []).map((p) => {
      ...
      return {
        ...
        name: a(p, 'Name', ''),
        configuration: (cfgNode?.Config || []).map((c) => ({ key: a(c, 'Key', ''), value: a(c, 'Value', '') })),
...
function buildNetworkInfo(net) {
  ...
  return [{ $: omitEmpty({ BindAddress: net.bindAddress, ExternalAddress: net.externalAddress, Port: net.port }) }];
}

function buildGrpc(grpc) {
  if (!grpc || !grpc.port) return undefined;
  const node = { $: omitEmpty({ BindAddress: grpc.bindAddress, ExternalAddress: grpc.externalAddress, Port: grpc.port }) };
...
function buildXmlObject(cfg) {
  ...
  if (cfg.logging?.logs?.length) {
    ...
    const loggingNode = { Log: cfg.logging.logs.map((l) => ({ $: { Type: l.type, Destination: l.destination, Level: l.level } })) };
  ...
  if (ep) {
    ...
    if (ep.metricsEndpoint) epNode.MetricsEndpoint = [{ $: { Url: ep.metricsEndpoint.url, ApiKey: ep.metricsEndpoint.apiKey } }];
    ...
    if (time.ages?.length) {
      timeNode.Ages = [{
        Age: time.ages.map((age) => ({
          $: omitEmpty({ Name: age.name, StartDate: age.startDate, EndDate: age.endDate, StartYear: age.startYear }),
        })),
      }];
    }
  ...
  if (handlers) {
    ...
    if (handlers.death) {
      ...
      if (d.coma?.value) dNode.Coma = [{ _: d.coma.value, $: omitEmpty({ Timeout: d.coma.timeout, Effect: d.coma.effect }) }];
    ...
    if (handlers.chat) {
      hNode.Chat = [{
        $: omitEmpty({
          ...
          CommandPrefix: handlers.chat.commandPrefix !== '/' ? handlers.chat.commandPrefix : undefined,
        }),
      }];
    }
    ...
    root.Plugins = [{
      Message: [{
        Plugin: cfg.plugins.message.map((p) => {
          const pNode = { $: { Type: p.type, Passthrough: p.passthrough ? 'true' : 'false', Name: p.name } };
          if (p.configuration?.length) {
            pNode.Configuration = [{ Config: p.configuration.map((c) => ({ $: { Key: c.key, Value: c.value } })) }];
          }
          if (p.targets?.length) {
