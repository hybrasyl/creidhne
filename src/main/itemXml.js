...
function parsePhysical(node) {
  ...
  return {
    value: a(node, 'Value', '1'),
    weight: a(node, 'Weight', '1'),
    durability: a(node, 'Durability', '1')
  }
}
...
function parseCategories(node) {
  ...
  return (node.Category || []).map((c) =>
    typeof c === 'string'
      ? { name: c, unique: false }
      : { name: c._ ?? c, unique: toBool(a(c, 'Unique'), false) }
  )
}
...
function parseVendor(node) {
  if (!node) return { shopTab: '', description: '' }
...
function parseDamage(node) {
  if (!node) return null
  return {
    smallMin: a(node, 'SmallMin', '0'),
    smallMax: a(node, 'SmallMax', '0'),
    largeMin: a(node, 'LargeMin', '0'),
    largeMax: a(node, 'LargeMax', '0')
  }
}
...
function buildXmlObject(item) {
  ...
  const appAttrs = omitEmpty({
    ...
    DisplaySprite:
      p.appearance.displaySprite !== '' ? String(p.appearance.displaySprite) : undefined,
  ...
  if (p.categories.length)
    propsObj.Categories = [
      {
        Category: p.categories.map((c) =>
          c.unique ? { _: c.name, $: { Unique: 'true' } } : c.name
        )
      }
    ]
  ...
  propsObj.Physical = [
    {
      $: omitEmpty({
        Value: String(p.physical.value),
        Weight: String(p.physical.weight),
        Durability: String(p.physical.durability)
      })
    }
  ]
    ...
    propsObj.Equipment = [
      {
        $: omitEmpty({
          ...
          WeaponType: p.equipment.weaponType !== 'None' ? p.equipment.weaponType : undefined
        })
      }
    ]
  ...
  if (p.variants.groups.length || p.variants.names.length) {
    ...
    if (p.variants.names.length)
      varNode.Name = p.variants.names.map((n) => ({ $: { Group: n.group }, _: n.value }))
  ...
  if (p.damage)
    propsObj.Damage = [
      {
        $: {
          SmallMin: String(p.damage.smallMin),
          SmallMax: String(p.damage.smallMax),
          LargeMin: String(p.damage.largeMin),
          LargeMax: String(p.damage.largeMax)
        }
      }
    ]
  ...
  if (p.motions.length)
    propsObj.Motions = [
      {
        Motion: p.motions.map((m) => ({
          $: omitEmpty({ Id: String(m.id), Speed: String(m.speed) })
        }))
      }
    ]
    ...
    propsObj.Procs = [
      {
        Proc: p.procs.map((pr) => ({
          $: omitEmpty({
            Type: pr.type,
            Castable: pr.castable || undefined,
            Script: pr.script || undefined,
            Chance: String(pr.chance)
          })
        }))
      }
    ]
...
function buildCastModifier(cm) {
  const node = {
    $: omitEmpty({
      Group: cm.group || undefined,
      Castable: cm.castable || undefined,
      All: cm.all ? 'true' : undefined
    })
  }
...
function buildOp(op) {
  return {
    $: omitEmpty({
      Match: op.match !== '-1' ? String(op.match) : undefined,
      Amount: String(op.amount),
      Min: op.min !== '-1' ? String(op.min) : undefined,
      Max: op.max !== '255' ? String(op.max) : undefined
    })
  }
}
...
function buildUse(use) {
  ...
  if (use.teleport)
    node.Teleport = [
      { $: { X: String(use.teleport.x), Y: String(use.teleport.y) }, _: use.teleport.map }
    ]
  if (use.effect)
    node.Effect = [
      {
        $: omitEmpty({
          Id: String(use.effect.id),
          Speed: use.effect.speed !== '100' ? String(use.effect.speed) : undefined
        })
      }
    ]
  ...
  if (adds.length || removes.length) {
    const statNode = {}
    if (adds.length)
      statNode.Add = adds.map((s) => ({
        $: omitEmpty({
          Duration: String(s.duration),
          Intensity: String(s.intensity),
          Tick: String(s.tick),
          RemoveChance: s.removeChance || undefined,
          PersistDeath: s.persistDeath ? 'true' : undefined
        }),
        _: s.name
      }))
    if (removes.length)
      statNode.Remove = removes.map((s) => ({
        $: omitEmpty({
          IsCategory: s.isCategory ? 'true' : undefined,
