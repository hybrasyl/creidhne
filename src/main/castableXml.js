...
function mapRequirements(reqsNode, xmlString) {
  ...
  return (node.Requirement || []).map((req, idx) => {
    ...
    if (activePrereq) {
      ...
      prerequisites = block
        ? mapCommentedPrerequisites(block)
        : {
            deprecated: true,
            forbidCookie: '',
            forbidMessage: '',
...
const EMPTY_PLAYER = {
  peasant: { ...DEFAULT_MOTION },
  warrior: { ...DEFAULT_MOTION },
  wizard: { ...DEFAULT_MOTION },
  priest: { ...DEFAULT_MOTION },
  rogue: { ...DEFAULT_MOTION },
  monk: { ...DEFAULT_MOTION }
}
...
function mapHeal(effectsNode) {
  ...
  if (sq.kind === 'variable')
    return { kind: 'Variable', value: '', min: sq.min, max: sq.max, formula: '' }
  if (sq.kind === 'static')
    return { kind: 'Static', value: sq.value, min: '', max: '', formula: '' }
  return null
}
...
function mapDamage(effectsNode) {
  ...
  const flagsStr =
    flagsEl !== null ? (typeof flagsEl === 'string' ? flagsEl : (flagsEl?._ ?? '')) : ''
  ...
  if (formulaEl !== null) {
    const f = typeof formulaEl === 'string' ? formulaEl : (formulaEl?._ ?? '')
    if (f)
      return { kind: 'Formula', type: damageType, flags, value: '', min: '', max: '', formula: f }
  }
...
function mapXmlToCastable(result, xmlString) {
  ...
  return {
    ...
    damage:
      damage && meta.damageFormulaName
        ? { ...damage, formulaName: meta.damageFormulaName }
        : damage,
...
export function parseCastableXml(xmlString) {
  return new Promise((resolve, reject) => {
    xml2js.parseString(xmlString, { trim: true }, (err, result) => {
      ...
      try {
        resolve(mapXmlToCastable(result, xmlString))
      } catch (e) {
        reject(e)
      }
    })
...
function buildXmlObject(castable) {
    ...
    root.Intents = [
      {
        Intent: castable.intents.map((intent) => {
          ...
          if (intent.crosses?.length)
            node.Cross = intent.crosses.map((c) => ({
              $: { Radius: c.radius, VisualEffect: c.visualEffect }
            }))
          if (intent.cones?.length)
            node.Cone = intent.cones.map((c) => ({
              $: { Radius: c.radius, Direction: c.direction, VisualEffect: c.visualEffect }
            }))
          if (intent.squares?.length)
            node.Square = intent.squares.map((s) => ({
              $: { Side: s.side, VisualEffect: s.visualEffect }
            }))
          if (intent.lines?.length)
            node.Line = intent.lines.map((l) => ({
              $: { Length: l.length, Direction: l.direction, VisualEffect: l.visualEffect }
            }))
          if (intent.tiles?.length)
            node.Tile = intent.tiles.map((t) => ({
              $: {
                Direction: t.direction,
                RelativeX: t.relativeX,
                RelativeY: t.relativeY,
                VisualEffect: t.visualEffect
              }
            }))
          return node
        })
  ...
  if (castable.requirements?.length) {
    root.Requirements = [
      {
        Requirement: castable.requirements.map((req) => {
          const reqAttrs = {}
          ...
          if (pr && !pr.deprecated && hasPrContent) {
            ...
            reqNode.Prerequisites = [
              {
                $: pa,
                Prerequisite: (pr.castables || []).map((c) => ({
...

  {
    ...
    // Animations
    {
      ...
      const buildGroup = (group) => {
        if (!group) return null
        const node = {}
        const motions = CLASSES.filter((cls) => group.player?.[cls.toLowerCase()]?.id).map(
          (cls) => {
...
function _replaceNth(str, search, replacement, n) {
  let count = -1
  return str.replace(new RegExp(_escapeRegex(search), 'g'), (m) =>
    ++count === n ? replacement : m
  )
}
...
export function serializeCastableXml(castable) {
  ...
  let xml = injectComment(
    builder.buildObject(buildXmlObject(castable)),
    castable.comment,
    'Castable'
  )
  ...
  if (castable.requirements?.length) {
    castable.requirements.forEach((req, i) => {
      ...
      const prAttrs = _attrsStr({
        ForbidCookie: pr.forbidCookie,
        RequireCookie: pr.requireCookie,
        ForbidMessage: pr.forbidMessage,
        RequireMessage: pr.requireMessage
      })
