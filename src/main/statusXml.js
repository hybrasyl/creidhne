...
function mapDamage(damNode) {
  ...
  if (sq.kind === 'static' && sq.value)
    return { element, type, flags, kind: 'Static', value: sq.value, min: '', max: '', formula: '' }
  return null
}
...
export function parseStatusXml(xmlString) {
  return new Promise((resolve, reject) => {
    ...
    xml2js.parseString(xmlString, { trim: true }, (err, result) => {
      ...
      try {
        resolve(mapXmlToStatus(result, comment, meta))
      } catch (e) {
        reject(e)
      }
    })
...
function buildHeal(heal) {
  ...
  else if (heal.kind === 'Variable')
    node.Simple = [{ $: { Min: heal.min || '0', Max: heal.max || '0' } }]
...
function buildDamage(damage) {
  ...
  else if (damage.kind === 'Variable')
    node.Simple = [{ $: { Min: damage.min || '0', Max: damage.max || '0' } }]
...
function buildEffectBlock(block) {
  ...
  const msgs = buildMessages(block.messages)
  if (msgs) node.Messages = msgs
  const heal = buildHeal(block.heal)
  if (heal) node.Heal = heal
  const dam = buildDamage(block.damage)
  if (dam) node.Damage = dam
  const sm = buildStatModifiers(block.statModifiers)
  if (sm) node.StatModifiers = sm
  const cond = buildConditions(block.conditions)
  if (cond) node.Conditions = cond
  const hdlr = buildHandler(block.handler)
  if (hdlr) node.Handler = hdlr
...
function buildXmlObject(status) {
  ...
  const effects = {}
  const onApply = buildEffectBlock(status.onApply)
  if (onApply) effects.OnApply = onApply
  const onTick = buildEffectBlock(status.onTick)
  if (onTick) effects.OnTick = onTick
  const onRemove = buildEffectBlock(status.onRemove)
  if (onRemove) effects.OnRemove = onRemove
  const onExpire = buildEffectBlock(status.onExpire)
  if (onExpire) effects.OnExpire = onExpire
